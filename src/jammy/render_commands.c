	...
	const jm_render_command_draw_text* cmd)
{
	...
	const jm_font_info* fontInfo = jm_font_get_info(cmd->fontHandle);
	...
	for (uint32_t i = 0; i < textLength; ++i)
	{
		...
		if (!shouldBreak && potentionalBreakPoint)
		{
			...
			const float wordWidth = jm_font_measure_text_range(cmd->fontHandle, wordStart, wordEnd);
			if (penX + wordWidth > cmd->x + cmd->width)
			{
				shouldBreak = true;
			}
		}
		...
		if (glyph->hasBitmap && i >= cmd->rangeStart && i < cmd->rangeEnd)
		{
			const float x = penX + glyph->bitmap_left * cmd->scale;
			const float y = penY - glyph->bitmap_top * cmd->scale;
			const float w = glyph->width * cmd->scale;
			const float h = glyph->height * cmd->scale;
		...
		penX += glyph->advance_x * cmd->scale;
	}
